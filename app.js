...
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbx61iCeQYfX-OWPiesROz2Fw3pSkqX7pKYm8rd3_fItTfzlc9qle8eO0y8On1OBjBZkvQ/exec";
...
function getShiftForAgent(agentCode, dateStr) {
    ...
    if (existing && existing.shift) {
        // Cas particulier : si c'est un dimanche et que le shift stocké est 'C', on retourne 'R'
        const date = new Date(cleanDate);
        if (date.getDay() === 0 && existing.shift === 'C') {
            return 'R';
        }
...
async function saveLeaveWithJoker() {
    ...
    for (const dateStr of dates) {
      ...
      if (isSunday) {
    // Le dimanche devient C (congé) mais l'affichage final sera R via getShiftDisplay
    daysToProcess.push({ dateStr, agentShift: 'C', jokerShift: theoreticalShift, isSunday: true });
} else {
    ...
    // ==================== PARTIE MODIFIÉE DE saveLeaveWithJoker ====================
for (const day of daysToProcess) {
    const monthKey = day.dateStr.substring(0,7);
    if (!planningData[monthKey]) planningData[monthKey] = {};
    if (!planningData[monthKey][agentCode]) planningData[monthKey][agentCode] = {};

    // Sauvegarde du shift original (pour restauration ultérieure)
    const originalShift = getShiftForAgent(agentCode, day.dateStr);

    // Pour les dimanches : on stocke 'C' (même si affiché 'R')
    let shiftToStore = day.agentShift;
    if (day.isSunday) {
        shiftToStore = 'C';   // ← clé : stocker 'C' pour permettre la suppression
    }
...
// ==================== FIN DE LA MODIFICATION ====================
  ...
  saveData();
await saveSharedPlanning();
// Après saveSharedPlanning()
const anneeConges = new Date(dates[0]).getFullYear();
recalculerSoldesAgent(agentCode, anneeConges);   // recalc uniquement l'année concernée
await saveSharedSoldes();

// Rafraîchir l'affichage des soldes si la page est ouverte
if (document.getElementById('main-content').innerHTML.includes('Solde des congés')) {
...
function displayMainMenu() {
    ...
    // 🔍 RECHERCHE RAPIDE CODE PANIQUE (uniquement pour ADMIN et CP)
    if (currentUser.role === 'ADMIN' || currentUser.role === 'CP') {
        html += `<div style="margin: 15px 0; padding: 10px; background: #34495e; border-radius: 8px;">
            <div style="display: flex; gap: 10px; align-items: center; flex-wrap: wrap;">
                <span style="font-size: 1.2rem;">🔍</span>
                <input type="text" id="quickPanicSearch" placeholder="Rechercher un code panique (ex: 854, CP, O9)..." 
                       style="flex: 1; padding: 10px; border-radius: 5px; border: none; background: #2c3e50; color: white;"
                       onkeyup="searchPanicCodeQuick()">
            </div>
            <div id="quickPanicResult" style="margin-top: 10px;"></div>
        </div>`;
    }
...
async function refreshAllData() {
    ...
    try {
        ...
        showSnackbar("❌ Erreur lors de la synchronisation");
    }
}
...
function showNotificationsPanel() {
    ...
    // Supprimer l'ancien panneau en toute sécurité
    const oldPanel = document.getElementById('notificationsPanel');
    if (oldPanel) {
        try {
            if (oldPanel.parentNode) oldPanel.parentNode.removeChild(oldPanel);
            else if (oldPanel.remove) oldPanel.remove();
        } catch(e) { console.warn("Erreur suppression panneau:", e); }
    }
    ...
    if (closeBtn) {
        closeBtn.onclick = () => {
            ...
            } catch(e) { console.warn("Erreur fermeture:", e); }
        };
    }
...
function recalculerSoldesAgent(agentCode, anneeForcee = null) {
    const agent = agents.find(a => a.code === agentCode);
    ...
    const anneeActuelle = new Date().getFullYear();
    const toutesAnnees = [];
    for (let annee = anneeEntree; annee <= anneeActuelle; annee++) toutesAnnees.push(annee);

    // Si une année spécifique est forcée, on calcule depuis l'année d'entrée jusqu'à cette année
    let anneesACalculer = toutesAnnees;
    if (anneeForcee && toutesAnnees.includes(anneeForcee)) {
        anneesACalculer = toutesAnnees.filter(a => a <= anneeForcee);
    }

    let reportPrecedent = 0;

    for (const annee of anneesACalculer) {
        const droit = getDroitAnnuelPourAnnee(agent, annee);

        // Calcul auto depuis le planning (basé sur l'affichage des 'C')
        let prisAuto = 0;
        for (let mois = 1; mois <= 12; mois++) {
            const daysInMonth = new Date(annee, mois, 0).getDate();
            for (let d = 1; d <= daysInMonth; d++) {
        ...
        // Si cette année est l'année forcée, on impose la valeur du planning
        if (anneeForcee === annee) {
            pris = prisAuto;
        } else {
            // Sinon, on garde la valeur manuelle si elle existe, sinon la valeur auto
    ...
    <tr style="background-color:#2c3e50;">
        <td colspan="7" style="padding:10px;">
            <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
                ...
                ${isAdmin ? `<button class="popup-button green" onclick="recalculerToutSoldes('${agentCode}')">💾 Recalculer tout</button>` : ''}
            </div>
        </th>
    </tr>
</tfoot>
...
async function deleteLeavePeriod(agentCode, startDate, endDate) {
    ...
    if (confirm(`Supprimer les congés du ${formatDateToFrench(startDate)} au ${formatDateToFrench(endDate)} pour ${agentCode} ?`)) {
        ...
        // 1. Supprimer toutes les entrées de congé (y compris les dimanches, car stockés en 'C')
        while (current <= end) {
            ...
            // Supprimer le remplacement joker
            for (const joker of agents.filter(a => a.groupe === 'J')) {
                const jokerEntry = planningData[monthKey]?.[joker.code]?.[dateStr];
                if (jokerEntry && jokerEntry.type === 'remplacement_joker' && jokerEntry.comment && jokerEntry.comment.includes(agentCode)) {
        ...
        // 2. Restaurer les shifts théoriques (les dimanches retrouvent leur vrai shift)
        current = new Date(startDate);
        while (current <= end) {
            const dateStr = formatDateUTC(current);
            const monthKey = dateStr.substring(0, 7);
            const shiftTheorique = getTheoreticalShift(agentCode, dateStr);
            if (!planningData[monthKey]) planningData[monthKey] = {};
            if (!planningData[monthKey][agentCode]) planningData[monthKey][agentCode] = {};
            planningData[monthKey][agentCode][dateStr] = {
                shift: shiftTheorique,
                type: 'theorique',
                comment: 'Rétabli après suppression de congé',
                replaces: null
            };
            current.setDate(current.getDate() + 1);
        }

        // Nettoyer les notifications de remplacement
        replacementNotifications = replacementNotifications.filter(notif =>
            !(notif.agent_absent === agentCode &&
              ((notif.date_debut <= endDate && notif.date_fin >= startDate)))
        );

   saveData();
await saveSharedPlanning();
const anneeConges = new Date(startDate).getFullYear();
recalculerSoldesAgent(agentCode, anneeConges);
await saveSharedSoldes();

if (document.getElementById('main-content').innerHTML.includes('Solde des congés')) {
    afficherSoldesAgent(agentCode);
}
        addNotification('leave_delete', {
            action: 'delete',
            agentCode: agentCode,
            agentName: `${agents.find(a => a.code === agentCode)?.nom || agentCode} ${agents.find(a => a.code === agentCode)?.prenom || ''}`,
            startDate: startDate,
        ...
        alert("✅ Congés supprimés et planning restauré !");

        congesList = congesList.filter(c =>
            !(c.agentCode === agentCode && c.startDate === startDate && c.endDate === endDate)
        );
        await saveSharedConges();

        // Rafraîchir l'affichage (planning ou soldes)
        const mainContent = document.getElementById('main-content');
        if (mainContent && mainContent.innerHTML.includes('Solde des congés')) {
            afficherSoldesAgent(agentCode);
        } else {
            displayPlanningMenu();
        }
...
function showPanicList() {
    ...
    let html = `<div class="info-section"><h3>📋 Codes Panique</h3>
        <div style="display:flex; gap:10px; margin-bottom:15px;">
            <input type="text" id="searchPanicInput" placeholder="🔍 Rechercher par code, agent ou nom..." style="flex:1; padding:8px; border-radius:5px; border:1px solid #34495e; background:#2c3e50; color:white;" onkeyup="filterPanicList()">
        </div>
...
function generatePanicList(data) {
    if (!data.length) {
        return '<p style="text-align:center; padding:20px; color:#e74c3c;">🔍 Aucun code panique trouvé</p>';
    }
    return `<table class="classement-table" style="width:100%;">
        <thead><tr style="background-color:#34495e;">
            <th style="padding:8px;">Agent</th><th style="padding:8px;">Code</th><th style="padding:8px;">Poste/Secteur</th><th style="padding:8px;">Commentaire</th><th style="padding:8px;">Actions</th>
        </tr></thead>
        <tbody>${data.map(p => {
...
function filterPanicList() {
    const input = document.getElementById('searchPanicInput');
    if (!input) return;
    
    const term = input.value.toLowerCase().trim();
    let filtered = panicCodes.filter(p => {
        if (!canAccessAgent(p.agent_code)) return false;
        const agent = agents.find(a => a.code === p.agent_code);
        
        // Convertir le code en chaîne (au cas où il serait stocké comme nombre)
        const codeStr = String(p.code);
        const agentCodeStr = String(p.agent_code);
        
        const matchCode = codeStr.toLowerCase().includes(term);
        const matchAgentCode = agentCodeStr.toLowerCase().includes(term);
        const matchName = agent && (agent.nom.toLowerCase().includes(term) || agent.prenom.toLowerCase().includes(term));
        
        return matchCode || matchAgentCode || matchName;
    });
    
    const container = document.getElementById('panicListContainer');
    if (container) {
        container.innerHTML = generatePanicList(filtered);
    }
}
...
// ==================== RECHERCHE RAPIDE CODE PANIQUE ====================
// ==================== RECHERCHE RAPIDE CODE PANIQUE (BIDIRECTIONNELLE) ====================
function searchPanicCodeQuick() {
    const input = document.getElementById('quickPanicSearch');
    const resultDiv = document.getElementById('quickPanicResult');
    ...
    // Définition des correspondances Code ↔ Poste
    const correspondances = [
        { code: "854", poste: "CP", description: "Chef de patrouille" },
                { code: "828", poste: "O1a", description: "Agent " },
                                { code: "913", poste: "O1b", description: "Agent o1b et L 5 " },
                                { code: "513", poste: "O3", description: "Agent " }, 
                              { code: "313", poste: "O4", description: "Agent " },  
                                                                  { code: "832", poste: "Rlog", description: "RLOG" }, 
                
                                { code: "848", poste: "O5", description: "Agent O5 " },
                                
                        { code: "824", poste: "O8", description: "Agent " },
                                      { code: "911", poste: "O10", description: "Agent " },
                                                      { code: "326", poste: "O11", description: "Agent " },
                                                                     { code: "855", poste: "O12", description: "Agent " },
                                                                                    { code: "826", poste: "O13", description: "Agent O13etL18" },
                                                    { code: "838", poste: "O14", description: "Agent " },                                       { code: "113", poste: "O15", description: "Agent " },                        { code: "827", poste: "O16", description: "Agent " },                      { code: "????", poste: "L1", description: "Agent " }, 
                                                  { code: "126", poste: "L2", description: "Agent " }, 
                                                  { code: "842", poste: "L4", description: "Agent " },                       { code: "913", poste: "L5", description: "AgentL52et o1b " },
                                                   { code: "841", poste: "L6", description: "Agent " },
                                                  { code: "327", poste: "L7", description: "Agent " }, 
                                                   { code: "815", poste: "L8", description: "Agent " }, 
                                                  { code: "914", poste: "L9", description: "Agent " },
                                                   { code: "926", poste: "L10", description: "Agent " },
                                                  { code: "125", poste: "L11", description: "Agent " },
                                                  { code: "118", poste: "L13", description: "Agent " }, 
                                                     { code: "111", poste: "L14", description: "Agent " }, 
                                                                   { code: "999", poste: "L15", description: "Agent " },
                                                                    { code: "328", poste: "L16", description: "Agent " },                             { code: "826", poste: "L18", description: "Agent L13 et oscar13" }, 
                                                                    { code: "111", poste: "L20", description: "Agent " }, 
                                                  { code: "111", poste: "I2", description: "Agent " },                      { code: "???", poste: "I2", description: "Agent " },                            
        { code: "813", poste: "O9", description: "Agent O9" }
    ];
    
    let resultat = null;
    
    // 1. Recherche par code (ex: "854" → trouve CP)
    const parCode = correspondances.find(c => c.code === term);
    if (parCode) {
        resultat = {
            type: "code",
            code: parCode.code,
            poste: parCode.poste,
            description: parCode.description
        };
    }
    
    // 2. Recherche par poste (ex: "CP" → trouve 854)
    if (!resultat) {
        const parPoste = correspondances.find(c => c.poste === term);
        if (parPoste) {
            resultat = {
                type: "poste",
                code: parPoste.code,
                poste: parPoste.poste,
                description: parPoste.description
            };
        }
    }
    
    // 3. Si toujours pas trouvé, chercher dans la base panicCodes existante
    if (!resultat) {
        const foundInPanic = panicCodes.find(p => {
            const codeStr = String(p.code);
            const agentCode = p.agent_code.toUpperCase();
            const agent = agents.find(a => a.code === p.agent_code);
            const agentName = agent ? (agent.nom + ' ' + agent.prenom).toUpperCase() : '';
            return codeStr === term || agentCode === term || agentName.includes(term);
        });
        
        if (foundInPanic) {
            const agent = agents.find(a => a.code === foundInPanic.agent_code);
            resultat = {
                type: "direct",
                code: foundInPanic.code,
                poste: foundInPanic.poste || agent?.poste || foundInPanic.agent_code,
                description: agent ? `${agent.nom} ${agent.prenom}` : foundInPanic.comment,
                comment: foundInPanic.comment
            };
        }
    }
    ...
    if (resultat) {
        let html = `<div style="background: linear-gradient(135deg, #27ae60, #1e8449); padding: 12px 15px; border-radius: 10px; margin-top: 10px; box-shadow: 0 2px 5px rgba(0,0,0,0.2);">
            <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                <span style="font-size: 1.5rem;">🚨</span>
                <div style="flex: 1;">`;
        
        if (resultat.type === "code") {
            html += `<strong>🔑 Code panique : <span style="background:#e74c3c; padding:2px 10px; border-radius:15px; font-size:1.2rem;">${resultat.code}</span></strong><br>
                     📍 Poste correspondant : <strong style="background:#f39c12; padding:2px 10px; border-radius:15px; color:#2c3e50;">${resultat.poste}</strong><br>
                     📝 Description : ${resultat.description}<br>`;
        } 
        else if (resultat.type === "poste") {
            html += `<strong>📍 Poste : <span style="background:#f39c12; padding:2px 10px; border-radius:15px; color:#2c3e50;">${resultat.poste}</span></strong><br>
                     🔑 Code panique associé : <span style="background:#e74c3c; padding:2px 10px; border-radius:15px; font-size:1.2rem;">${resultat.code}</span><br>
                     📝 Description : ${resultat.description}<br>`;
        }
        else if (resultat.type === "direct") {
            html += `<strong>🔑 Code panique : <span style="background:#e74c3c; padding:2px 10px; border-radius:15px; font-size:1.2rem;">${resultat.code}</span></strong><br>
                     📍 Poste : ${resultat.poste}<br>
                     👤 Agent : ${resultat.description}<br>
                     💬 Commentaire : ${resultat.comment || '-'}<br>`;
        }
        
        html += `<button onclick="showPanicList()" style="background: #f39c12; border: none; padding: 5px 12px; border-radius: 20px; color: #2c3e50; cursor: pointer; font-weight: bold; margin-top: 8px;">📋 Voir tous les codes</button>
                </div>
            </div>
        </div>`;
        ...
        resultDiv.innerHTML = `
            <div style="background: #e74c3c; padding: 10px 15px; border-radius: 10px; margin-top: 10px;">
                ❌ Aucun résultat pour "<strong>${escapeHtml(term)}</strong>"
            </div>
        `;
    }
}
function searchPanicCode() {
    const term = document.getElementById('searchPanicInput')?.value.toLowerCase().trim() || '';
    const resultDiv = document.getElementById('searchPanicResult');
    if (!resultDiv) return;
    ...
    resultDiv.innerHTML = results.length ? generatePanicList(results) : '<p style="text-align:center; padding:20px;">🔍 Aucun code panique trouvé</p>';
}
