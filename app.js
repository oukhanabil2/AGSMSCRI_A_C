...
function recalculerSoldesAgent(agentCode) {
    ...
    for (let annee = anneeEntree; annee <= anneeActuelle; annee++) {
        ...
        // Calcul des jours pris depuis planningData
        let pris = 0;
        for (let mois = 1; mois <= 12; mois++) {
            const daysInMonth = new Date(annee, mois, 0).getDate();
            for (let d = 1; d <= daysInMonth; d++) {
        ...
        // Priorité à la valeur manuelle si définie
        const existant = soldesConges.find(s => s.agentCode === agentCode && s.annee === annee);
        if (existant && existant.pris !== undefined && !isNaN(existant.pris)) {
            pris = existant.pris;
        }

        const totalDispo = droit + reportPrecedent;
        const reste = totalDispo - pris;   // ← autorise les valeurs négatives
